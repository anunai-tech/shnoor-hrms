  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-2xl font-bold text-gray-800">Contact Queries</h1>
          <p className="font-body text-sm text-gray-400 mt-1">
            Messages from the landing page contact form
            {unreadCount > 0 && (
              <span className="font-display ml-2 bg-yellow-400 text-white text-xs font-bold px-2 py-0.5 rounded-full">
                {unreadCount} unread
              </span>
            )}
          </p>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100">
        <div className="px-6 py-4 border-b border-gray-100 flex gap-2">
          {['All', 'Unread', 'Read', 'Replied'].map(s => (
            <button key={s} onClick={() => setStatusFilter(s)}
              className={`font-display px-4 py-1.5 rounded-lg text-xs font-medium transition
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan="7" className="font-body text-center py-10 text-sm text-gray-400">No queries found</td></tr>
              ) : (
                filtered.map((query, index) => (
                  <tr key={query.id}
                    ...
                    <td className="px-6 py-4">
                      <p className={`font-display text-sm ${query.status === 'Unread' ? 'font-semibold text-gray-800' : 'font-medium text-gray-700'}`}>
                        {query.name}
                      </p>
                    </td>
                    <td className="font-body px-6 py-4 text-sm text-gray-500">{query.email}</td>
                    <td className="font-body px-6 py-4 text-sm text-gray-600">{query.subject}</td>
        ...
        <div className="px-6 py-3 border-t border-gray-100">
          <p className="font-body text-xs text-gray-400">Showing {filtered.length} of {queries.length} queries</p>
        </div>
      </div>
        ...
        <Modal title="Contact Query" onClose={() => setShowViewModal(false)}>
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="font-display text-xs text-gray-400 font-medium mb-1">From</p>
                <p className="font-display text-sm font-semibold text-gray-800">{selectedQuery.name}</p>
              </div>
              <div>
                <p className="font-display text-xs text-gray-400 font-medium mb-1">Date</p>
                <p className="font-body text-sm text-gray-600">{new Date(selectedQuery.created_at).toLocaleDateString('en-GB')}</p>
              </div>
              <div>
                <p className="font-display text-xs text-gray-400 font-medium mb-1">Email</p>
                <p className="font-body text-sm text-gray-600">{selectedQuery.email}</p>
              </div>
              <div>
                <p className="font-display text-xs text-gray-400 font-medium mb-1">Status</p>
                <Badge status={selectedQuery.status} />
              </div>
            </div>
            <div>
              <p className="font-display text-xs text-gray-400 font-medium mb-1">Subject</p>
              <p className="font-display text-sm font-semibold text-gray-800">{selectedQuery.subject}</p>
            </div>
            <div>
              <p className="font-display text-xs text-gray-400 font-medium mb-1">Message</p>
              <p className="font-body text-sm text-gray-600 leading-relaxed bg-gray-50 rounded-lg p-4">{selectedQuery.message}</p>
            </div>
          </div>
          <div className="flex gap-3 mt-6">
            {selectedQuery.status !== 'Replied' && (
              <button onClick={() => markReplied(selectedQuery.id)}
                className="font-display flex-1 bg-yellow-400 hover:bg-yellow-500 text-white text-sm font-semibold py-2.5 rounded-lg transition">
                Mark as Replied
              </button>
            )}

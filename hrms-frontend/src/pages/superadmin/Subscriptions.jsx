...
function Subscriptions() {
  ...
  const handleAdd = async () => {
    try { await createSubscription(formData); setShowAddModal(false); setFormData({ name: '', monthly_price: '', annual_price: '', max_users: '' }); fetchPlans() }
    catch (err) { console.error(err) }
  }

  const handleEdit = async () => {
    try { await updateSubscription(selectedPlan.id, formData); setShowEditModal(false); fetchPlans() }
    catch (err) { console.error(err) }
  }

  const handleDelete = async () => {
    try { await deleteSubscription(selectedPlan.id); setShowDeleteModal(false); fetchPlans() }
    catch (err) { console.error(err) }
  }
  ...
  const formFields = (
    <div className="space-y-4">
      <div>
        <label className="font-display block text-sm font-medium text-gray-700 mb-1">Plan Name</label>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="font-display block text-sm font-medium text-gray-700 mb-1">Monthly Price (₹)</label>
          <input name="monthly_price" type="number" value={formData.monthly_price} onChange={handleFormChange}
            className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400" />
        </div>
        <div>
          <label className="font-display block text-sm font-medium text-gray-700 mb-1">Annual Price (₹)</label>
          <input name="annual_price" type="number" value={formData.annual_price} onChange={handleFormChange}
            className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400" />
        </div>
      </div>
      <div>
        <label className="font-display block text-sm font-medium text-gray-700 mb-1">Max Users</label>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-2xl font-bold text-gray-800">Subscriptions</h1>
          <p className="font-body text-sm text-gray-400 mt-1">Manage subscription plans for companies</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100">
          ...
          <table className="w-full">
            <thead>
              <tr className="text-xs text-gray-400 border-b border-gray-100 bg-gray-50">
                ...
                <th className="font-display text-left px-6 py-3 font-medium">Plan Name</th>
                <th className="font-display text-left px-6 py-3 font-medium">Monthly Price</th>
                <th className="font-display text-left px-6 py-3 font-medium">Annual Price</th>
                <th className="font-display text-left px-6 py-3 font-medium">Max Users</th>
            ...
            <tbody>
              {plans.map((plan, index) => (
                <tr key={plan.id} className="border-b border-gray-50 hover:bg-gray-50 transition">
                  ...
                  <td className="font-display px-6 py-4 text-sm font-semibold text-gray-800">{plan.name}</td>
                  <td className="font-body px-6 py-4 text-sm text-gray-600">₹{Number(plan.monthly_price).toLocaleString('en-IN')}</td>
                  <td className="font-body px-6 py-4 text-sm text-gray-600">₹{Number(plan.annual_price).toLocaleString('en-IN')}</td>
                      ...
                      {plan.name !== 'Default' && (
                        <>
                          ...
                          <button onClick={() => { setSelectedPlan(plan); setShowDeleteModal(true) }}
                            className="font-display text-xs text-red-500 hover:underline font-medium">Delete</button>
                        </>
                      )}
                    </div>
                  </td>
        ...
        <div className="px-6 py-3 border-t border-gray-100">
          <p className="font-body text-xs text-gray-400">{plans.length} plans total</p>
        </div>
      </div>
      ...
      {showAddModal && (
        <Modal title="Add New Plan" onClose={() => setShowAddModal(false)}>
          ...
          <div className="flex gap-3 mt-6">
            <button onClick={handleAdd} className="font-display flex-1 bg-yellow-400 hover:bg-yellow-500 text-white text-sm font-semibold py-2.5 rounded-lg transition">Add Plan</button>
      ...
      {showDeleteModal && (
        <Modal title="Delete Plan" onClose={() => setShowDeleteModal(false)}>
          <p className="font-body text-sm text-gray-600">Are you sure you want to delete <span className="font-display font-semibold">{selectedPlan?.name}</span>?</p>
          <div className="flex gap-3 mt-6">

...
function SuperAdminProfile() {
  ...
  const handlePhotoUpload = async (e) => {
    ...
    if (!allowed.includes(file.type)) { setPicError('Only image files are allowed (jpg, png, webp, gif)'); return }
    if (file.size > 5 * 1024 * 1024) { setPicError('Image must be smaller than 5 MB'); return }
    ...
    try {
      ...
      const res = await api.post('/superadmin/profile-picture', formPayload, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
      if (res.data.data?.url) {
        setProfilePicUrl(res.data.data.url + '?t=' + Date.now())
  ...
  const handleRemovePhoto = async () => {
    ...
    try {
      await api.delete('/superadmin/profile-picture')
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div className="flex items-center gap-6 mb-8 pb-6 border-b border-gray-100">
          <div className="relative">
            <div className="w-24 h-24 rounded-full overflow-hidden bg-yellow-100 flex items-center justify-center">
              {picUploading ? (
                ...
                <span className="text-yellow-600 font-black text-4xl">{formData.first_name?.charAt(0) || 'A'}</span>
              )}
            </div>
          ...
          <div>
            ...
            <span className="inline-block mt-2 bg-yellow-50 text-yellow-600 text-xs font-semibold px-3 py-1 rounded-full">Super Admin</span>
            {profilePicUrl && !picUploading && (
              <button onClick={handleRemovePhoto} className="block mt-1 text-xs text-red-400 hover:text-red-600 transition">
                Remove photo
              </button>
            )}
          </div>
        ...
        {picError && <div className="bg-red-50 border border-red-200 text-red-600 text-sm rounded-lg px-4 py-2 mb-5">{picError}</div>}

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">First Name</label>
            {isEditing ? <input name="first_name" value={formData.first_name} onChange={handleChange} className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400" /> : <p className="text-sm text-gray-700">{formData.first_name}</p>}
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Last Name</label>
            {isEditing ? <input name="last_name" value={formData.last_name} onChange={handleChange} className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400" /> : <p className="text-sm text-gray-700">{formData.last_name}</p>}
          </div>
          <div>
          ...
          <div>
            ...
            {isEditing ? <input name="phone" value={formData.phone} onChange={handleChange} className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400" /> : <p className="text-sm text-gray-700">{formData.phone || '—'}</p>}
          </div>
          <div>
          ...
          <div>
            ...
            {isEditing ? <input name="address" value={formData.address} onChange={handleChange} className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400" /> : <p className="text-sm text-gray-700">{formData.address || '—'}</p>}
          </div>
        </div>
      </div>
    </div>

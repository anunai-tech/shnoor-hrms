    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="font-body text-sm text-gray-400 mt-1">Manage your personal information</p>
        </div>
        {!isEditing ? (
          ...
          <div className="flex gap-3">
            <button onClick={handleSave} disabled={saving}
              className="font-display bg-yellow-400 hover:bg-yellow-500 disabled:bg-yellow-200 text-white text-sm font-semibold px-5 py-2.5 rounded-lg transition">
              {saving ? 'Saving...' : 'Save Changes'}
            </button>
            <button onClick={() => setIsEditing(false)}
              className="font-display border border-gray-200 text-gray-600 text-sm font-medium px-5 py-2.5 rounded-lg hover:bg-gray-50 transition">
              Cancel
            </button>
          </div>
        )}
      ...
      {saved && (
        <div className="font-body bg-green-50 border border-green-200 text-green-600 text-sm rounded-lg px-4 py-3">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div className="flex items-center gap-6 mb-8 pb-6 border-b border-gray-100">
          <div className="relative">
            <div className="w-24 h-24 rounded-full overflow-hidden bg-yellow-100 flex items-center justify-center">
              {profilePhoto ? (
                ...
                <span className="font-display text-yellow-600 font-black text-4xl">
            ...
            <label className="absolute bottom-0 right-0 w-7 h-7 bg-yellow-400 rounded-full flex items-center justify-center cursor-pointer hover:bg-yellow-500 transition">
              <span className="font-display text-white text-xs font-bold">+</span>
          ...
          <div>
            ...
            <span className="font-display inline-block mt-2 bg-yellow-50 text-yellow-600 text-xs font-semibold px-3 py-1 rounded-full">
              Super Admin
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <div>
            ...
            ) : <p className="font-body text-sm text-gray-700">{formData.first_name}</p>}
          </div>
          <div>
            ...
            {isEditing ? (
              <input name="last_name" value={formData.last_name} onChange={handleChange}
                className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400" />
            ) : <p className="font-body text-sm text-gray-700">{formData.last_name}</p>}
          </div>
          <div>
            <label className="font-display block text-sm font-medium text-gray-700 mb-1">Email</label>
            <p className="font-body text-sm text-gray-400">{user?.email} <span className="text-xs">(cannot change)</span></p>
          </div>
          <div>
            ...
            {isEditing ? (
              <input name="phone" value={formData.phone} onChange={handleChange}
                className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400" />
            ) : <p className="font-body text-sm text-gray-700">{formData.phone || '—'}</p>}
          </div>
          <div>
            <label className="font-display block text-sm font-medium text-gray-700 mb-1">Role</label>
            <p className="font-body text-sm text-gray-400">Super Admin <span className="text-xs">(cannot change)</span></p>
          </div>
          <div>
            <label className="font-display block text-sm font-medium text-gray-700 mb-1">Address</label>
            {isEditing ? (
              <input name="address" value={formData.address} onChange={handleChange}
                className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400" />
            ) : <p className="font-body text-sm text-gray-700">{formData.address || '—'}</p>}
          </div>
        </div>
      </div>
    </div>

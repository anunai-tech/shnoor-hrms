...
function Leaves() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-display text-2xl font-bold text-gray-800">Leaves</h1>
        <p className="font-body text-sm text-gray-400 mt-1">Manage employee leave requests</p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100">
          ...
          <table className="w-full">
            <thead>
              <tr className="text-xs text-gray-400 border-b border-gray-100 bg-gray-50">
                ...
                <th className="font-display text-left px-6 py-3 font-medium">From</th>
                <th className="font-display text-left px-6 py-3 font-medium">To</th>
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan="8" className="font-body text-center py-10 text-sm text-gray-400">No leaves found</td></tr>
              ) : (
        ...
        <div className="px-6 py-3 border-t border-gray-100">
          <p className="font-body text-xs text-gray-400">Showing {filtered.length} of {leaves.length} requests</p>
        </div>
      </div>
        ...
        <Modal title="Leave Request" onClose={() => setShowViewModal(false)}>
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div><p className="text-xs text-gray-400 font-medium mb-1">Employee</p><p className="font-display text-sm font-semibold text-gray-800">{selectedLeave.first_name} {selectedLeave.last_name}</p></div>
              <div><p className="text-xs text-gray-400 font-medium mb-1">Leave Type</p><p className="font-body text-sm text-gray-700">{selectedLeave.leave_type}</p></div>
              <div><p className="text-xs text-gray-400 font-medium mb-1">From</p><p className="font-body text-sm text-gray-700">{new Date(selectedLeave.start_date).toLocaleDateString('en-GB')}</p></div>
              <div><p className="text-xs text-gray-400 font-medium mb-1">To</p><p className="font-body text-sm text-gray-700">{new Date(selectedLeave.end_date).toLocaleDateString('en-GB')}</p></div>
              <div><p className="text-xs text-gray-400 font-medium mb-1">Days</p><p className="font-body text-sm text-gray-700">{selectedLeave.days}</p></div>
          ...
          {selectedLeave.status === 'Pending' && (
            <div className="flex gap-3 mt-6">
              <button onClick={() => handleStatus(selectedLeave.id, 'Approved')}
                className="font-display flex-1 bg-green-500 hover:bg-green-600 text-white text-sm font-semibold py-2.5 rounded-lg transition">Approve</button>
              <button onClick={() => handleStatus(selectedLeave.id, 'Rejected')}
                className="font-display flex-1 bg-red-500 hover:bg-red-600 text-white text-sm font-semibold py-2.5 rounded-lg transition">Reject</button>
            </div>
          )}

      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan="8" className="font-body text-center py-10 text-sm text-gray-400">No expenses found</td></tr>
              ) : (
        ...
        <div className="px-6 py-3 border-t border-gray-100">
          <p className="font-body text-xs text-gray-400">Showing {filtered.length} of {expenses.length} claims</p>
        </div>
      </div>
        ...
        <Modal title="Expense Claim" onClose={() => setShowViewModal(false)}>
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div><p className="text-xs text-gray-400 font-medium mb-1">Employee</p><p className="font-display text-sm font-semibold text-gray-800">{selectedExpense.first_name} {selectedExpense.last_name}</p></div>
              ...
              <div><p className="text-xs text-gray-400 font-medium mb-1">Title</p><p className="font-body text-sm text-gray-700">{selectedExpense.title}</p></div>
              <div><p className="text-xs text-gray-400 font-medium mb-1">Category</p><p className="font-body text-sm text-gray-700">{selectedExpense.category}</p></div>
              <div><p className="text-xs text-gray-400 font-medium mb-1">Date</p><p className="font-body text-sm text-gray-700">{new Date(selectedExpense.created_at).toLocaleDateString('en-GB')}</p></div>

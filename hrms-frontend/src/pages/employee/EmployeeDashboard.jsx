...
function EmployeeDashboard() {
  ...
  useEffect(() => {
    loadAttendance()
    // Fetch shift independently so the shift card shows even before clocking in
    api.get('/employee/my-shift').then(res => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div className="flex flex-wrap gap-3 mb-5">
          ...
          {isClockedIn && !isClockedOut && (
            isOnLunch ? (
              <button onClick={() => handleAction('/employee/lunch-end', 'Failed to end break')}
              ...
              <button onClick={() => handleAction('/employee/lunch-start', 'Failed to start break')}
                disabled={actionLoading}
        ...
        {/* Progress bar — shift timings from attendance record, then standalone shift fetch, then nothing */}
      ...
      {/* Shift Info — uses today's attendance record if clocked in, otherwise falls back to the standalone shift fetch */}

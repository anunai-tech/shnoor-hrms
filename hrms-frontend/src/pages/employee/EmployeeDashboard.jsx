  ...
  useEffect(() => {
    ...
    getMyLeaves()
      .then(res => {
        ...
        const paidUsed = approved.filter(l => l.leave_type === 'Paid Leave').reduce((sum, l) => sum + Number(l.days), 0)
        const sickUsed = approved.filter(l => l.leave_type === 'Sick Leave').reduce((sum, l) => sum + Number(l.days), 0)
        const casualUsed = approved.filter(l => l.leave_type === 'Casual Leave').reduce((sum, l) => sum + Number(l.days), 0)
        setLeaveBalance([

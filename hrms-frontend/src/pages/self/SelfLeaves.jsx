...
function SelfLeaves() {
  ...
  const leaveBalance = [
    { type: 'Paid Leaves', total: 12, used: leaves.filter(l => l.leave_type === 'Paid Leave' && l.status === 'Approved').reduce((s, l) => s + l.days, 0) },
    { type: 'Sick Leaves', total: 6, used: leaves.filter(l => l.leave_type === 'Sick Leave' && l.status === 'Approved').reduce((s, l) => s + l.days, 0) },
    { type: 'Casual Leaves', total: 6, used: leaves.filter(l => l.leave_type === 'Casual Leave' && l.status === 'Approved').reduce((s, l) => s + l.days, 0) },
  ]
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {leaveBalance.map(lb => (
          <div key={lb.type} className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
            ...
            <p className="text-3xl font-bold text-gray-800 mt-2">{lb.total - lb.used}</p>

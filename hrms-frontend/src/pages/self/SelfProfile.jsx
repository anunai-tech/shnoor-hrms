...
function SelfProfile() {
  ...
  const handleSave = async () => {
    try {
      setSaving(true)
      const res = await updateManagerProfile({ ...formData, profile_photo: profilePhoto })
  ...
  return (
    <div className="space-y-6">
      ...
      {saved && <div className="font-body bg-green-50 border border-green-200 text-green-600 text-sm rounded-lg px-4 py-3">Profile updated successfully!</div>}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div className="flex items-center gap-6 mb-8 pb-6 border-b border-gray-100">
          <div className="relative">
            <div className="w-24 h-24 rounded-full overflow-hidden bg-amber-100 flex items-center justify-center">
              {profilePhoto ? (
                ...
                <span className="text-primary font-black text-4xl">{formData.first_name?.charAt(0) || 'M'}</span>
              )}
            </div>
          ...
          <div>
            ...
            <span className="font-display inline-block mt-2 bg-amber-50 text-amber-700 text-xs font-semibold px-3 py-1 rounded-full">Manager</span>
          </div>
        </div>

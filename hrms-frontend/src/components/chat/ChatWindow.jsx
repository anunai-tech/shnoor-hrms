...
function ChatWindow({
  ...
  const handleEditSave = async (msgId) => {
    ...
    if (onEditMessage) await onEditMessage(msgId, editInputValue)
  ...
  if (!conversation) {
    return (
      <div className="flex flex-1 min-h-0 items-center justify-center bg-gradient-to-br from-slate-50 via-blue-50/40 to-white px-6 text-center">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-sm text-slate-500">
              {conversation.designation || conversation.role || conversation.email}
            </p>
          </div>
        </div>
      </div>
            ...
            {messages.map(message => {
              ...
              return (
                <div key={message.id} className={`flex ${isOwnMessage ? 'justify-end' : 'justify-start'}`}>
                  <div className={`group relative max-w-[85%] rounded-[28px] px-4 py-3 shadow-sm sm:max-w-[75%] border border-slate-200 bg-white text-slate-800 ${
                    isOwnMessage ? 'rounded-tr-none' : 'rounded-tl-none'
                  }`}>
                    {!isEditing && isOwnMessage && message.message && (
                      <button onClick={() => handleEditInit(message)}
                    ...
                    {attachmentUrl && hasImageAttachment && (
                      <a href={attachmentUrl} target="_blank" rel="noreferrer" className="mt-3 block overflow-hidden rounded-2xl">
                        <img src={attachmentUrl} alt={message.file_name || 'Attachment'} className="max-h-64 w-full rounded-2xl object-cover" />
                      </a>
                    )}
